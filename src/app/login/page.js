...
export default function LoginPage() {
  ...
  const handleLogin = (e) => {
    ...
    setAlert({ message: `Welcome back, ${loggedInUser.name}!`, type: "success" });

    // Redirect based on role
    setTimeout(() => {
      if (loggedInUser.role === "admin") router.push("/dashboard/admin");
      else if (loggedInUser.role === "owner") router.push("/dashboard/owner");
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-blue-500 via-indigo-500 to-purple-500 p-6">
      <div className="bg-white/20 backdrop-blur-md p-10 rounded-3xl shadow-2xl w-full max-w-md transform transition duration-500 hover:scale-105">
        ...
        <p className="mt-6 text-center text-white/90">
          ...
          <a href="/register" className="text-yellow-300 hover:underline font-medium">
            Register
          </a>
        </p>
      </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        disabled={disabled}
        className={`w-full px-4 py-3 rounded-xl border border-white/30 bg-white/20 text-white placeholder-white/70 focus:outline-none focus:ring-2 focus:ring-yellow-300 transition ${
  ...
  return (
    <div className={`fixed top-6 right-6 px-6 py-4 rounded-lg shadow-lg ${bgColor} z-50`}>
      <div className="flex items-center justify-between">
        <span>{message}</span>
        <button onClick={onClose} className="ml-4 font-bold text-xl leading-none hover:opacity-70 transition">
          &times;
        </button>
      </div>
    </div>
