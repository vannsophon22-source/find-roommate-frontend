...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setAlert({ 
        message: `Welcome ${userName} (${role})! Redirecting...`, 
        type: "success" 
      });

      // Redirect based on role WITHOUT storing anything
      setTimeout(() => {
        if (role === "admin") {
      ...
      <div className="bg-white/20 backdrop-blur-md p-10 rounded-3xl shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold py-3 rounded-xl transition ${
        ...
        <div className="mt-6 p-4 bg-white/10 rounded-xl">
          ...
          <div className="text-xs text-white/70 space-y-1">
            <p>• Regular User: any email → /dashboard/user/homepage</p>
            <p>• Admin: admin@example.com → /dashboard/admin</p>
            <p>• Owner: owner@example.com → /dashboard/owner</p>
            <p className="mt-1">Password: any value accepted</p>
          </div>
        </div>
      </div>
    </div>
